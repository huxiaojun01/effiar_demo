...
function notifyReceivedMessage(message) {
	...
	if (message.format == '1') {
		...
		message.content = "img[" + "/files/chat-space/" + image.image + "]"
	}
	if (message.format == '2') {
		var voice = JSON.parse(message.content);
		message.content = "audio[" + "/files/chat-space/" + voice.key + "]"
	}
	if (message.format == '3') {
		var file = JSON.parse(message.content);
		message.content = "file(" + "/files/chat-space/" + file.key + ")["
				+ file.name + "]"
	}
	if (message.format == '8') {
		var video = JSON.parse(message.content);
		message.content = "video[" + "/files/chat-space/" + video.video + "]"
	}
	if (message.format == '4') {
		var map = JSON.parse(message.content);
		
		message.content = "位置: "+map.address +" 经度: "+map.longitude+" 纬度: "+map.latitude;
	}
	var from = type == 'friend' ? findFriend(message.sender): findFriend(message.extra);
	var fromMessage = {
		...
		,
		cid : message.id // 消息id，可不传。除非你要对消息进行一些操作（如撤回）
		,
		mine : false // 是否我发送的消息，如果为true，则会显示在右方
		,
		fromid : message.extra // 消息的发送者id（比如群组中的某个消息发送者），可用于自动解决浏览器多窗口时的一些问题
		,
		timestamp : message.timestamp
	// 服务端时间戳。注意：JS中的时间戳是精确到毫秒，如果你返回的是标准的 unix 时间戳，记得要 *1000
	};
	mLayim.getMessage(fromMessage);

	receiptMessage(message.id);
}
